    ...
    >
      <div style={{ width: '100%', maxWidth: '380px' }}>
        ...
        <div className="flex items-center gap-2 mb-10 justify-center">
          ...
          <span className="text-[17px] font-extrabold" style={{ color: 'var(--fg)' }}>Truleado</span>
        </div>
        ...
        >
          <h1 className="text-[20px] font-extrabold tracking-[-0.5px] mb-1" style={{ color: 'var(--fg)' }}>
            Waitlist admin
          </h1>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            ...
            />
            {error && (
              <p className="text-[13px]" style={{ color: 'var(--adv)' }}>{error}</p>
            )}
            <button
              type="submit"
              disabled={loading || !password}
              className="w-full rounded-lg py-[13px] text-[14px] font-bold text-white border-none cursor-pointer transition-opacity duration-200"
              style={{
                background: 'var(--acc)',
